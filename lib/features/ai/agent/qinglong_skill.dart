...
- 只读先行、写在后面。多个互不依赖的查询/收集/统计（包括 ssh/exec 这类可读可写的终端命令）可以交给 `parallel_tools` 一批并发执行统一拿结果；写操作注意不要并发改同一资源，需要确认时仍会走确认策略。
...
- `parallel_tools`：一批互不依赖的工具调用并行执行（同时查多个脚本/任务/日志/接口，
  或同时向多台机器跑 ssh/exec），不需要子代理的独立分析时用它更省更直接；要各自深度分析再考虑 `parallel_agents`。
