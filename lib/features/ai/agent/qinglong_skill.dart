...
- APP 会在展示模型输出前自动运行插件里 `process(text)` / `transform(text)` 函数，专门清理输出里泄露的内部调用标记（例如 `<｜tool｜ calls>`、`<｜tool｜ invoke>`）。
- 这是**展示层整理**，不改变任何真实执行：你看到正文时插件已经处理过了。
- 用户说"帮我写/改输出整理插件"时，先读提供商设置里配置的插件路径，再按普通 JS 文件编辑；文件头必须保留 `@qinglong-plugin` 注释，并导出 `process` 或 `transform` 函数。
