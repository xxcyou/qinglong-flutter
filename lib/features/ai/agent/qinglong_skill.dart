...
- 只读先行、写在后面。多个互不依赖的只读查询/收集/统计，优先用 `parallel_tools` 一批并发执行统一拿结果；写操作一个一个来，每写完核实一次，不要并行写。
...
- `parallel_tools`：只是“一批只读工具调用”并行执行（同时查多个脚本/任务/日志/接口），
  不需要子代理的独立分析时用它更省更直接；要各自深度分析再考虑 `parallel_agents`。
