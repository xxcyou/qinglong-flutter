    ...
    tools.add(
      ExternalTool(
        ...
        description: '通用条件执行/微流程引擎 v2。一次调用按顺序执行多个步骤，'
            ...
            '- {"type":"for","var":"i","start":0,"end":5,"step":1,"body":[...]} 或 {"items":"\$list","var":"item","body":[...]}: 循环（不写 var 默认 item；items 传对象时循环 \$item.key/\$item.value）\n'
            '- {"type":"while","while":"表达式","max":1000,"body":[...]}: 条件循环\n'
            '- {"type":"break"}、{"type":"break","level":2} 或 {"type":"break","label":"outer"}: 跳出当前/多层/指定标签循环；'
