...
class ChatNotifier extends Notifier<ChatState> {
  ...
  Future<String> _runConditionExec(
    ...
    if (rawSteps is String) {
      try {
        rawSteps = jsonDecode(rawSteps);
      } catch (e) {
        return '条件执行失败：steps 不是合法 JSON。$e';
      }
    }
  ...
  void _emitWorkflowStep({
    ...
    _appendAgentEvent(
      ...
      AgentEvent(
        ...
        args: args,
        result: _conditionSnippet(result ?? '', 800),
        fullResult: result == null
            ? null
            : result.length > AgentEvent.persistedFullResultLimit
                ? '${result.substring(0, AgentEvent.persistedFullResultLimit)}'
                    '\n…（完整内容过长，展示时截断）'
                : result,
        durationMs: durationMs,
        ok: ok,
  ...
  static String _conditionSnippet(String text, [int max = 240]) {
    ...
    final tools = <ExternalTool>[
      ...
      ExternalTool(
        ...
        parameters: const {
          ...
          'properties': {
            ...
            'brightness': {
              'type': 'string',
              'enum': ['light', 'dark'],
              'description': 'create 时亮度，默认 dark'
            },
            'background_image': {
              'type': 'string',
              'description':
                  '背景图 guest 路径，如 /workspace/wallpapers/ocean.png；空=纯配色'
            },
            'primary': {'type': 'string', 'description': '主色 hex，如 #66BB6A'},
        ...
        invoke: (args) async {
          ...
          switch (action) {
            case 'list':
              if (state.themes.isEmpty) return '还没有主题方案。';
              return [
                '共 ${state.themes.length} 个主题方案：',
                for (final t in state.themes)
                  '${t.id} | ${t.name} | ${t.isDark ? '暗色' : '亮色'}'
                      '${t.backgroundImage.isEmpty ? '' : ' | 背景:${t.backgroundImage}'}'
                      ' | 主色:${t.colors['primary'] ?? ''}'
                      ' | 强调:${t.colors['accent'] ?? ''}'
            ...
            case 'create':
              ...
              final brightness =
                  args['brightness']?.toString().trim() == 'light'
                      ? 'light'
                      : 'dark';
              final stamp = DateTime.now().millisecondsSinceEpoch;
              final newId = 'theme_$stamp';
              final colors = {
                ...ThemeConfig.defaultColorsForBrightness(brightness),
                if ((args['primary']?.toString().trim() ?? '').isNotEmpty)
                  'primary': args['primary'].toString().trim(),
                if ((args['accent']?.toString().trim() ?? '').isNotEmpty)
                  'accent': args['accent'].toString().trim(),
                if ((args['background']?.toString().trim() ?? '').isNotEmpty)
                  'background': args['background'].toString().trim(),
                if ((args['on_surface']?.toString().trim() ?? '').isNotEmpty)
                  'onSurface': args['on_surface'].toString().trim(),
              };
              final theme = ThemeConfig(
                ...
                backgroundImage:
                    args['background_image']?.toString().trim() ?? '',
                colors: colors,
          ...
          return '未知 action：$action（可用 list/apply/get/create/import/remove）。';
        },
      ),
      ...MetaTools.build(
    ...
    tools.add(
      ExternalTool(
        ...
        description: '通用条件执行/微流程引擎 v2。一次调用按顺序执行多个步骤，'
            ...
            '- {"type":"for","var":"i","start":0,"end":5,"step":1,"body":[...]} 或 {"items":"\$list","var":"item","body":[...]}: 循环（不写 var 默认 item；items 传对象时循环 \$item.key/\$item.value；save_to 可接收 break 返回值）\n'
            ...
            'params 值为数组表示对象参数解构（如 user 解出 \$name/\$age）\n'
            ...
            'map/filter/reduce/keys/values/entries/'
            ...
            'map(\$list,"fn")/filter(\$list,"fn")/reduce(\$list,"fn")/keys(\$obj)/values(\$obj)/entries(\$obj) 做集合管道。'
